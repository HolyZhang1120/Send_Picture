...
u8 stdbmp_decode(const u8 *filename) 
{
	...
	if(res==0)//打开成功.
	{ 
		pic_size =  f_size(f_bmp);
		printf("pic_size1:0x%x",pic_size);
//		readlen = rev_picture(0,NULL,pic_size,0);
		while(rev_picture(0,NULL,pic_size,0) == NULL)
		{
			delay_ms(10);
...
//		for(ki=0;ki<readlen;ki++)
//			printf("%02x",databuf[ki]);
		pbmp=(BITMAPINFO*)databuf;					//得到BMP的头部信息 
		
//		pic_size = pbmp->bmfHeader.bfSize;
//		printf("%x",pic_size);
		crc_pic = CRC32Software((u8 *)databuf,readlen);
		while(rev_picture(1,(u8 *)databuf,readlen,pack) == NULL)
		{
			delay_ms(10);
			atk_8266_quit_trans();//退出透传
			atk_8266_send_cmd("AT+CIPCLOSE","OK",50);         //关闭连接
...
//			for(ki=0;ki<readlen;ki++)
//				printf("%02x",databuf[ki]);
			crc_pic = CRC32Software_((u8 *)databuf,readlen,(uint32_t)crc_pic);
			while(rev_picture(1,(u8 *)databuf,readlen,pack) == NULL)
			{
				atk_8266_quit_trans();//退出透传
				atk_8266_send_cmd("AT+CIPCLOSE","OK",50);         //关闭连接
		...
		if(pic_flag)
		{
			while(rev_picture(2,NULL,(u32)crc_pic,0) == NULL)
			{
				atk_8266_quit_trans();//退出透传
				atk_8266_send_cmd("AT+CIPCLOSE","OK",50);         //关闭连接
				connect_server();
